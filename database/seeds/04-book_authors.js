exports.seed = function (knex) {
            return knex('book_authors').insert([
                { book_id: 1, author_id: 1 },
                { book_id: 1, author_id: 2 },
                { book_id: 2, author_id: 3 },
                { book_id: 3, author_id: 4 },
                { book_id: 3, author_id: 5 },
                { book_id: 3, author_id: 6 },
                { book_id: 4, author_id: 7 },
                { book_id: 4, author_id: 8 },
                { book_id: 5, author_id: 9 },
                { book_id: 5, author_id: 10 },
                { book_id: 6, author_id: 10 },
                { book_id: 6, author_id: 11 },
                { book_id: 7, author_id: 12 },
                { book_id: 7, author_id: 13 },
                { book_id: 7, author_id: 14 },
                { book_id: 7, author_id: 15 },
                { book_id: 7, author_id: 16 },
                { book_id: 7, author_id: 17 },
                { book_id: 7, author_id: 18 },
                { book_id: 8, author_id: 19 },
                { book_id: 8, author_id: 20 },
                { book_id: 9, author_id: 21 },
                { book_id: 9, author_id: 22 },
                { book_id: 9, author_id: 23 },
            ]);
};
